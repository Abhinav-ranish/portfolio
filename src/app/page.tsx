...
export default function HomePage() {
  ...
  <main className="h-screen w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth bg-neutral-950 text-neutral-100">
      ...
      {showPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70 backdrop-blur-sm">
          <div className="bg-neutral-900 text-white rounded-xl p-6 max-w-sm mx-auto shadow-lg text-center">
            <h2 className="text-lg font-semibold text-pink-300 mb-2">Heads up!</h2>
            <p className="text-gray-300 text-sm mb-4">
              New UI for mobile is under development and will be available in the next 30 days.
            </p>
            <button
              onClick={() => {
                setShowPopup(false)
      ...
      {!isMobile && (
        <>
          {/* Hero with AI Chat */}
        <section id="hero" className="h-screen w-full flex flex-col justify-center snap-start">
            <Hero />
          </section>

          {/* Work Experience Timeline */}
          <section id="experience" className="w-full min-h-screen py-20 snap-start">
            <Experience />
          </section>
